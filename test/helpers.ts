...
type NodeTree = string | [string, NodeTree[]];
...
export function serializeNode(node: Node): NodeTree {
  if (node instanceof Element) {
    if (node.firstChild !== null) {
      return [
        serializeOpenTag(node),
        Array.from(node.childNodes, serializeNode),
      ];
    } else {
      return serializeOpenTag(node);
    }
  } else {
...
function serializeOpenTag(element: Element): string {
  const serializedHTML = (element.cloneNode() as Element).outerHTML;
  return serializedHTML.slice(0, serializedHTML.indexOf('>') + 1);
}
