...
export function vdom(...children: VChild[]): DirectiveElement<VChild[]> {
  return createDirectiveElement(VDOMDirective, children);
}
...
class VDOMBinding implements Binding<VChild[]> {
  ...
  private readonly _rootNode: BlockNode = createBlockNode(createNullBlock());
...
function createBlock(child: VChild, context: UpdateContext): Block {
  switch (typeof child) {
    case 'boolean':
    case 'undefined':
      return createNullBlock();
    case 'object': {
      if (child === null) {
        return createNullBlock();
      }
      if (Array.isArray(child)) {
        return createGroupBlock(child, context);
      }
      if (!isVElement(child)) {
      ...
      const directive =
        typeof child.type === 'function'
          ...
          : child.type;
      const element = createDirectiveElement(directive, child.props);
      const slot = context.resolveSlot(element, part);
...
function createGroupBlock(
  ...
  return {
    ...
    childNodes: childNodes.map((childNode) =>
      createBlockNode(createBlock(childNode, context)),
    ),
  };
}

function createNullBlock(): NullBlock {
  return {
    type: BlockType.Null,
    hostNode: document.createComment(''),
  };
...
function reconcileChildren(
  ...
  while (index < children.length) {
    const child = children[index]!;
    const node = createBlockNode(createBlock(child, context));
...
function updateNode(
  ...
  switch (node.block.type) {
    case BlockType.Directive:
      if (isVElement(child) && typeof child.type === 'function') {
        ...
        node.block.slot.reconcile(element, context);
        return;
      }
      break;
    case BlockType.Element:
  ...
  node.alternateBlock = createBlock(child, context);
}
