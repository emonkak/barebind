...
export class Reactive<T> extends Signal<T> {
  ...
  set value(value: T) {
    ...
    this._container.flags |= FLAG_NEW_VALUE | FLAG_PENDING_DIFFERENCE;
  ...
  get(key: PropertyKey, options?: ReactiveOptions): Reactive<unknown> | null {
    ...
    const child = getChildContainer(this._container, key);
...
function getChildContainer<T extends object>(
  ...
  child = resolveChildContainer(parent, key);
...
function proxyObject<T extends object>(
  ...
  return new Proxy(parent.source.value, {
    get(_target, key, _receiver) {
      const child = getChildContainer(parent, key);
      return getContainerValue(child);
    },
    set(target, key, value, receiver) {
      const child = getChildContainer(parent, key);
      if (child.source instanceof Atom) {
        child.flags |= FLAG_NEW_VALUE | FLAG_PENDING_DIFFERENCE;
...
function resolveChildContainer<T extends object>(
  parent: ReactiveContainer<T>,
  key: PropertyKey,
): ReactiveContainer<unknown> {
  ...
  if (flags & FLAG_NEEDS_SNAPSHOT) {
    ...
    if (isObject(oldSource)) {
      ...
      for (const [key, child] of children!.entries()) {
        if (
          child !== null &&
          child.flags & (FLAG_NEW_VALUE | FLAG_NEEDS_SNAPSHOT)
        ) {
          (newSource as any)[key] = takeSnapshot(child);
          child.flags &= ~FLAG_NEW_VALUE;
        }
      }
